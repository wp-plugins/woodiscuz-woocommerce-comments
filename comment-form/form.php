...
//    initialize the validator function
    validator.message['invalid'] = '<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_invalid_field']; ?>';
    validator.message['empty'] = '<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_error_empty_text']; ?>';
    ...
    jQuery(document).ready(function ($) {
        $(document).delegate('.wpc-toggle', 'click', function() {
            ...
            $('#wpc-comm-' + uniqueID + ' .wpc-reply').slideToggle(500, function () {
                if ($(this).is(':hidden')) {
                    $('#' + toggleID).html('<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_show_replies_text']; ?> &or;');
                } else {
                    $('#' + toggleID).html('<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_hide_replies_text']; ?> &and;');
                }
            });
        });
...
<div id="woopcomm">
    <p class="wpc-comment-title"><?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_header_text']; ?></p>
                ...
                <div id="wpc-form-footer-<?php echo $unique_id; ?>" class="wpc-form-footer">
                    <?php if (!is_user_logged_in()) { ?>
                        <div class="wpc-author-data">
                            <div class="wpc-field-name item"><input id="wpc_name-<?php echo $unique_id; ?>" class="wpc_name" name="wpc_name" required="required" value="" type="text" placeholder="<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_name_text'] ?>"/></div>
                            <div class="wpc-field-email item"><input id="wpc_email-<?php echo $unique_id; ?>" class="wpc_email email" name="wpc_email" required="required" value="" type="email" placeholder="<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_email_text']; ?>"/></div>
                            <div style="clear:both"></div>
                        </div>
                    <?php } ?>
                    <div class="wpc-form-submit">
                        <?php if (!$this->wpc_options->wpc_options_serialized->wpc_captcha_show_hide) { ?>
                            <?php if (!is_user_logged_in()) { ?>
                                <div class="wpc-field-captcha item">
                                    ...
                                    <span class="captcha_msg"><?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_captcha_text']; ?></span>
                                </div>
                            <?php } ?>
                        <?php } ?>
    ...
    <?php if ($wpc_parent_comments_count > $this->wpc_options->wpc_options_serialized->wpc_comment_count) { ?>
        <div class="wpc-load-more-submit-wrap">
            <input type="button" name="submit" value="<?php echo $this->wpc_options->wpc_options_serialized->wpc_phrases['wpc_load_more_submit_text']; ?>" id="wpc-load-more-submit-<?php echo $unique_id; ?>" class="wpc-load-more-submit button"/>
            <input type="hidden" name="wpc_comments_offset" id="wpc_comments_offset" value="1" />
            <input type="hidden" name="wpc_parent_per_page" id="wpc_parent_per_page" value="<?php echo $this->wpc_options->wpc_options_serialized->wpc_comment_count; ?>" />
